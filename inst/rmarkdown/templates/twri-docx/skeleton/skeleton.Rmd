...
Headings are automatically numbered. If you want a section heading to remain unnumbered use ` {-} ` after the heading. If you don't want any heading numbers, edit the yaml code block at the top of the rmarkdwon document as follows:

```yaml
output: 
  twriRmdTemplate::twri_rmd:
...
The [`flextable`](https://davidgohel.github.io/flextable/) package provides additional formatting flexibility when exporting to Word.

```{r}
ft <- flextable(head(dat, n = 10))
ft <- set_caption(ft, caption = "Flextable formatted mtcars dataset.")
ft
```
